...
build_firefox_addon() {
  ...
  files="$(find background_scripts content_scripts icons popup manifest.json -type f ! -name "*.ts" )"
  echo "$(echo "$files" | tr ' ' '\n' | awk '{print "  " $1}')"
...
build_chromium_addon() {
  ...
  files="$(find background_scripts content_scripts icons popup manifest.json service_worker.js -type f ! -name "*.ts")"
  echo "$(echo "$files" | tr ' ' '\n' | awk '{print "  " $1}')"
